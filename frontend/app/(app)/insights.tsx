...
// Supports youtu.be/ID, youtube.com/watch?v=ID, youtube.com/live/ID, with or
// without extra query params (e.g. ?si=... share tracking params).
function getYouTubeId(url: unknown): string | null {
  // Local require()'d video assets resolve to a number (or an object on web),
  // never a string — only ever check remote string URLs for a YouTube pattern.
...
// Note on video sources: pexels.com/download/... links are signed and expire,
// and public test buckets (like the Google one that failed earlier) get
// throttled or pulled without notice — neither is safe to hardcode.
//
// The URLs below are Pexels' actual CDN file URLs (videos.pexels.com/video-files/...),
// the same ones Pexels' own site hands to partners like Canva — not the
// "download" button link. They are plain, unauthenticated, direct .mp4 links,
// so they work with useVideoPlayer/VideoView as-is. Pexels does throttle
// bandwidth on these under heavy repeated/automated traffic, so for a real
// production app you should still eventually re-host your own copies (e.g. via
// the reels backend already in this file — POST /api/reels — using your
// uploaded kyoto.mp4, Margaret_River_Vineyard.mp4, Uluwatu_Cliffside.mp4).
const DUMMY_VIDEOS = [
  {
    _id: '1',
    ...
    likes: 2400,
    // Pexels: "Drone capture of vineyard landscape with a tractor" — pexels.com/video/vineyard-14629989/
    videoUrl: 'https://videos.pexels.com/video-files/14629989/14629989-uhd_2560_1440_25fps.mp4',
  }, {
    _id: '2',
    ...
    likes: 1800,
    // Pexels: "Stunning aerial shot of coastal cliffs and ocean waves" — pexels.com/video/bali-18023427/
    videoUrl: 'https://videos.pexels.com/video-files/18023427/18023427-uhd_2560_1440_25fps.mp4',
  }, {
    _id: '3',
    ...
    likes: 920,
    // Pexels: "Serene bamboo water fountain in lush Kyoto temple garden" — pexels.com/video/tranquil-kyoto-temple-bamboo-fountain-32272459/
    videoUrl: 'https://videos.pexels.com/video-files/32272459/13763929_1440_2560_50fps.mp4',
  }];
        ...
        <View style={[styles.miCard, { backgroundColor: cardBg, borderColor: theme.outlineVariant }]}>
          <View style={{ flexDirection: 'row', alignItems: 'center', marginBottom: 16 }}>
            ...
            <View style={{ marginLeft: 12 }}>
              <Text style={[typography.labelMd, { color: theme.onSurface, fontSize: 16, fontWeight: 'bold' }]}>Boolok GPT</Text>
...
const VideoItem = ({ item, isActive, cardHeight, cardWidth, isMobile, onDelete }: any) => {
  ...
  const hasPlayableSource = !youtubeId && !!item.videoUrl;

  // If this is a YouTube link or there's no video source at all (fallback demo
  // card), don't hand it to expo-video — pass null so the native player stays idle.
  const player = useVideoPlayer(hasPlayableSource ? item.videoUrl : null, (p) => {
    p.loop = true;
    if (isActive && hasPlayableSource) p.play();
  });

  useEffect(() => {
    if (!hasPlayableSource) return; // YouTube embed manages its own play/pause; no-source cards have nothing to play
    if (isActive) {
      player.play();
    } else {
      player.pause();
    }
  }, [isActive, hasPlayableSource]);

  const togglePlay = () => {
    if (!hasPlayableSource) return; // YouTube's own player controls handle playback; no-source cards have nothing to toggle
    if (player.playing) {
      player.pause();
    } else {
      player.play();
  ...
  return (
    ...
    >
      ...
      <Pressable onPress={togglePlay} style={StyleSheet.absoluteFill}>
        {youtubeId ? (
          // YouTube embed — uses YouTube's own player/controls, autoplays only
          // when this card is the active one in the feed (mirrors the mp4 behavior).
          <WebView
            source={{
              uri: `https://www.youtube.com/embed/${youtubeId}?playsinline=1&modestbranding=1&rel=0&autoplay=${isActive ? 1 : 0}&mute=${isActive ? 0 : 1}`,
            }}
            style={{ width: cardWidth, height: cardHeight, backgroundColor: '#000' }}
            allowsInlineMediaPlayback
            mediaPlaybackRequiresUserAction={false}
            javaScriptEnabled
          />
        ) : !hasPlayableSource ? (
          // No video source at all (fallback demo card, e.g. API unreachable) —
          // show a plain placeholder instead of an empty black VideoView.
          <View style={[{ width: cardWidth, height: cardHeight, backgroundColor: '#1a1a1a' }, styles.playOverlay]}>
            <MaterialCommunityIcons name="video-off-outline" size={40} color="rgba(255,255,255,0.35)" />
          </View>
        ) : (
          <>
            <VideoView
      ...
      <View style={[styles.rightOverlay, { bottom: isMobile ? 120 : 80 }]} pointerEvents="box-none">
      ...
      <LinearGradient
        ...
        style={[styles.bottomOverlay, { paddingBottom: isMobile ? 24 : 20 }]}
        pointerEvents="none"
      >
...
export default function InsightsScreen() {
  ...
  const cardHeight = isMobile ? height : cardWidth * (16 / 9);
        ...
        <View style={{ flex: 1, width: '100%' }}>
          ...
          <View style={styles.mobileTopBar}>
            ...
            <Pressable
              ...
              style={[styles.mobileUploadBtn, { backgroundColor: 'rgba(0,0,0,0.4)', borderColor: 'rgba(255,255,255,0.2)' }]}
            >
              <MaterialIcons name="add" size={18} color="#fff" />
              <Text style={{ color: '#fff', fontSize: 12, fontWeight: '600', marginLeft: 4 }}>Upload</Text>
            </Pressable>
          </View>
